...
async fn main() {
    ...
    let current_number_of_threads = Arc::new(Mutex::new(0));
    let process_finished_notifier = Arc::new(Notify::new());
    let service = make_service_fn(|_| {
        let current_number_of_threads = current_number_of_threads.clone();
        let current_process_finished_notifier = process_finished_notifier.clone();

        async move {
            Ok::<_, Infallible>(service_fn(move |req| {
                let current_number_of_threads = current_number_of_threads.clone();
                let current_process_finished_notifier = current_process_finished_notifier.clone();
                async move {
                    ...
                    match (req.method().clone(), &segments as &[&str]) {
                        (hyper::Method::POST, ["classic", machine_hash]) => {
                            ...
                            while *current_number_of_threads.lock().unwrap() >= MAX_THREADS_NUMBER {
                                current_process_finished_notifier.notified().await;
                            }
                            task::spawn({
                                *current_number_of_threads.lock().unwrap() += 1;
                                ...
                                async move {
                                    ...
                                    *current_number_of_threads.lock().unwrap() -= 1;
                                    current_process_finished_notifier.notify_waiters();
                                }
                            })
                            .await;
